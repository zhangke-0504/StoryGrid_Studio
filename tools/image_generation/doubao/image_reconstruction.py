...
from utils.create_model_response.openai.create_model_response import GPTClient
...
class OriginImagePromptAuditor:
	def __init__(self, openai_config_path: str = "config/openai/config.yaml"):
		self.gpt_client = GPTClient(config_path=_resolve_path(openai_config_path))
...
""".strip().format(language=language)
		...
		return await self.gpt_client.async_non_stream_response(
			prompt=prompt,
			instructions=instructions,
			text_format=PromptAuditExtractionResult,
		)
...
""".strip().format(language=language)
		...
		return await self.gpt_client.async_non_stream_response(
			prompt=prompt,
			instructions=instructions,
			text_format=ReconstructionPromptResult,
		)
